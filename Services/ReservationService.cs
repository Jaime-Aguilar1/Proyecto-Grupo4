...
public class ReservationService : IReservationService
{
    ...
    private IReservationService _reservationServiceImplementation;
    private IReservationService _reservationServiceImplementation1;
    ...
    public async Task<BillReservationDto> CalculateReservationCost(string roomId, DateTime checkIn, DateTime checkOut)
    {
        ...
        decimal nightPrice = roomSnap.GetValue<decimal>("tarifaBase"); 
        int nights = (checkOut - checkIn).Days;
        if (nights <= 0) nights = 1;

        decimal subTotal = nightPrice * nights;
        ...
        return new BillReservationDto
        {
            ...
            ISV = isv,
            TotalIsv = total,
            PayReserv = total * (decimal)0.50, // 50% para confirmar (lógica de negocio)
    ...
    public async Task<ConfirmReservationDto> CreateReservation(CreateReservationDto dto, string userId)
    {
        return await _firestoreDb.RunTransactionAsync(async transaction =>
        {
           ...
           // Actualizamos el usuario para cumplir con el PDF (hasReserved=true) [cite: 99]
